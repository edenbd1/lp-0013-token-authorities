...
pub enum Instruction {
    ...
    /// - Token Definition account (initialized, authorized),
    /// - Token Holding account (uninitialized or authorized and initialized),
    /// - Authority signer account (authorized).
    ...
    /// - Token Definition account (initialized, authorized),
    /// - Authority signer account (authorized).
    RotateAuthority { new_authority: AccountId },

    /// Permanently revoke the mint authority (makes supply fixed).
    ///
    /// Required accounts:
    /// - Token Definition account (initialized, authorized),
    /// - Authority signer account (authorized).
...
impl TokenDefinition {
    ...
    pub const fn authority(&self) -> Option<&Authority> {
        match self {
            Self::FungibleWithAuthority { authority, .. } => Some(authority),
            Self::Fungible { .. } | Self::NonFungible { .. } => None,
        }
    }

    pub const fn authority_mut(&mut self) -> Option<&mut Authority> {
        match self {
            Self::FungibleWithAuthority { authority, .. } => Some(authority),
            Self::Fungible { .. } | Self::NonFungible { .. } => None,
        }
...
impl TokenHolding {
    ...
    pub const fn zeroized_from_definition(
        ...
        match definition {
            TokenDefinition::Fungible { .. } | TokenDefinition::FungibleWithAuthority { .. } => {
                Self::Fungible {
                    definition_id,
                    balance: 0,
                }
